...
function createObjects() {
    ...
    // Alas (Bottom) - size W x D (4 x 2)
    const alasGroup = new THREE.Group();
    alasGroup.position.set(0, -H/2, 0); // Position at the bottom of the cuboid
    const alasMesh = new THREE.Mesh(new THREE.BoxGeometry(W, faceThickness, D), jaringMaterials[3]);
    alasMesh.position.set(0, 0, 0);
    alasGroup.add(alasMesh);
    jaringGroup.add(alasGroup);

    // Front (Depan) - size W x H (4 x 2), folds around z = D/2 (1)
    const frontHinge = new THREE.Group();
    frontHinge.position.set(0, 0, D/2);
    const frontMesh = new THREE.Mesh(new THREE.BoxGeometry(W, faceThickness, H), jaringMaterials[4]);
    frontMesh.position.set(0, 0, H/2);
    frontHinge.add(frontMesh);
    alasGroup.add(frontHinge);
    hinges.front = frontHinge;

    // Back (Belakang) - size W x H (4 x 2), folds around z = -D/2 (-1)
    const backHinge = new THREE.Group();
    backHinge.position.set(0, 0, -D/2);
    const backMesh = new THREE.Mesh(new THREE.BoxGeometry(W, faceThickness, H), jaringMaterials[5]);
    backMesh.position.set(0, 0, -H/2);
    backHinge.add(backMesh);
    alasGroup.add(backHinge);
    hinges.back = backHinge;

    // Top (Atas) - size W x D (4 x 2), folds around z = -H (-2) relative to back hinge
    const topHinge = new THREE.Group();
    topHinge.position.set(0, 0, -H);
    const topMesh = new THREE.Mesh(new THREE.BoxGeometry(W, faceThickness, D), jaringMaterials[2]);
    topMesh.position.set(0, 0, -D/2);
    topHinge.add(topMesh);
    backHinge.add(topHinge);
    hinges.top = topHinge;

    // Left (Kiri) - size H x D (2 x 2), folds around x = -W/2 (-2)
    const leftHinge = new THREE.Group();
    leftHinge.position.set(-W/2, 0, 0);
    const leftMesh = new THREE.Mesh(new THREE.BoxGeometry(H, faceThickness, D), jaringMaterials[1]);
    leftMesh.position.set(-H/2, 0, 0);
    leftHinge.add(leftMesh);
    alasGroup.add(leftHinge);
    hinges.left = leftHinge;

    // Right (Kanan) - size H x D (2 x 2), folds around x = W/2 (2)
    const rightHinge = new THREE.Group();
    rightHinge.position.set(W/2, 0, 0);
    const rightMesh = new THREE.Mesh(new THREE.BoxGeometry(H, faceThickness, D), jaringMaterials[0]);
...
function setMode(mode) {
    ...
    if (mode === 'sisi') {
        ...
        panel.innerHTML = "Jaring-jaring balok terdiri dari 6 buah persegi panjang. Tekan tombol <b>Buka Jaring-Jaring</b> di panel kiri untuk melihat proses membukanya secara interaktif.";
