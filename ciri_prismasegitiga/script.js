...
function createObjects() {
    ...
    // Help function to build 2D triangle mesh
    function createTriangleMesh(p1, p2, p3, material) {
    ...
    // Central face: Depan (Front) - size 4 x 4, index 2
    const frontMesh = new THREE.Mesh(new THREE.BoxGeometry(4, 4, faceThickness), jaringMaterials[2]);
    frontMesh.position.set(0, 0, 1.5); // centered at front of original prism
    jaringGroup.add(frontMesh);

    // Left face: Kiri Belakang - size 3.606 x 4, index 3
    const leftHinge = new THREE.Group();
    leftHinge.position.set(-2, 0, 1.5);
    const leftMesh = new THREE.Mesh(new THREE.BoxGeometry(3.606, 4, faceThickness), jaringMaterials[3]);
    leftMesh.position.set(-1.803, 0, 0);
    leftHinge.add(leftMesh);
    jaringGroup.add(leftHinge);
    hinges.left = leftHinge;

    // Right face: Kanan Belakang - size 3.606 x 4, index 4
    const rightHinge = new THREE.Group();
    rightHinge.position.set(2, 0, 1.5);
    const rightMesh = new THREE.Mesh(new THREE.BoxGeometry(3.606, 4, faceThickness), jaringMaterials[4]);
    rightMesh.position.set(1.803, 0, 0);
    rightHinge.add(rightMesh);
    jaringGroup.add(rightHinge);
    hinges.right = rightHinge;

    // Top face: Atas (Triangle), index 1
    const topHinge = new THREE.Group();
    topHinge.position.set(0, 2, 1.5);
    const topMesh = createTriangleMesh(
    ...
    // Bottom face: Bawah (Triangle), index 0
    const bottomHinge = new THREE.Group();
    bottomHinge.position.set(0, -2, 1.5);
    const bottomMesh = createTriangleMesh(
        new THREE.Vector2(-2, 0),
        new THREE.Vector2(2, 0),
        new THREE.Vector2(0, -3),
        jaringMaterials[0]
...
function setMode(mode) {
    ...
    if (mode === 'sisi') {
        ...
        panel.innerHTML = "Jaring-jaring prisma segitiga terdiri dari 3 persegi panjang dan 2 segitiga. Tekan tombol <b>Buka Jaring-Jaring</b> di panel kiri untuk melihat proses membukanya secara interaktif.";
...
function updateJaring() {
    ...
    const angleSide = foldProgress * (Math.PI - 0.9828); // Angle between front and side face (~123.7 degrees)
    const angleCap = foldProgress * Math.PI / 2;       // 90 degrees for caps

    if (hinges.left) hinges.left.rotation.y = -angleSide;
    if (hinges.right) hinges.right.rotation.y = angleSide;
